...
if(isset($_POST["register"])){
    ...
    $duplicateemail = mysqli_query($db, "SELECT * FROM utilizadores WHERE email = '$email'");
    $duplicateuser = mysqli_query($db, "SELECT * FROM utilizadores WHERE username = '$username'");
    ...
    if(mysqli_num_rows($duplicateemail) > 0 OR mysqli_num_rows($duplicateuser) > 0){
        ...
        if($password == $confirmpassword){
            $query = mysqli_query ($db,"INSERT INTO utilizadores (username,primeiroNome,ultimoNome,dataNascimento,genero,email,token,password,dataCriacao) VALUES ('$username','$firstName','$lastName','$date','$genre','$email','$token','$password','$currentDate')");
            ...
            $mail->Body = '
            <p>Bem-Vindo</p>
            <p><a href="http://localhost/atw/validar.php?token='.$token.'&email='.$_POST['email'].'">Clique aqui para validar o seu email</a></p>
            ';
            try{
                $mail->Send();
                echo
